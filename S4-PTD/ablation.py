...
from models.s4.s4_PTD_ablation import S4_PTD
...
parser.add_argument('--delta', default=0.2, type=float, help='Size of perturbation')
...
class S4Model(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            l = S4_PTD(d_model, dropout=dropout, delta=delta, transposed=True, lr=min(0.001, args.lr))
            cond, norm = l.getparams()
            self.conds = np.append(self.conds,cond)
            self.nrms = np.append(self.nrms,norm)
            self.s4_layers.append(l)
...
if __name__ == '__main__':
    ...
    for epoch in pbar:
        ...
        scheduler.step()
        losses_train = np.append(losses_train, train_acc)
        losses_val = np.append(losses_val, val_acc)
        losses_test = np.append(losses_test, test_acc)
        if epoch+1 % 100 == 0:
            conds, norms = model.getparams()
            dic = {'losses_train':losses_train, 'losses_val':losses_val, 'losses_test':losses_train, 'conds':conds, 'norms':norms}
            mlio.savemat('errors'+ str(args.delta) +'.mat',dic)